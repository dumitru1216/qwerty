...
#include "..\..\valve_sdk\bspflags.h"
#include "..\..\valve_sdk\ICvar.h"
#include "..\..\valve_utils\Math.h"
#include "..\..\valve_sdk\ClientClass.h"
#include <algorithm>
...
void TraceLine(Vector& vecAbsStart, Vector& vecAbsEnd, unsigned int mask, C_BaseEntity *ignore, C_Trace *ptr)
{
	...
	csgo_trace->TraceRay(C_Ray(vecAbsStart, vecAbsEnd), mask, &filter, ptr);
}
...
bool HandleBulletPenetration(WeaponInfo_t *wpn_data, FireBulletData &data, bool extracheck, Vector point)
{
	surfacedata_t *enter_surface_data = csgo_surface_data->GetSurfaceData(data.enter_trace.surface.surfaceProps);
	...
	if (!TraceToExitalt(dummy, data.enter_trace, data.enter_trace.end, data.direction, &trace_exit))
		return false;
	surfacedata_t *exit_surface_data = csgo_surface_data->GetSurfaceData(trace_exit.surface.surfaceProps);
